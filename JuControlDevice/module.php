...
class JuControlDevice extends IPSModule
{
    ...
    private const VAR_IDENT_DEVICE_TYPE                 = 'deviceType';
    private const VAR_IDENT_DEVICE_SN                   = 'deviceSN';
    ...
    private const SERVER_KNM  = 'https://www.myjudo.eu/interface';
    private const SERVER_JUDO = 'https://www.my-judo.com:8124';

    private const DT_I_SOFT_SAFE_PLUS   = '0x33'; //SAFE+
    ...
    private function RegisterVariables(string $deviceType): void
    {
        $position = -1;
    ...
    public function ApplyChanges(): void
    {
        ...
        if (in_array(
            '',
            [
                $this->ReadPropertyString(self::PROP_USERNAME),
    ...
    public function RequestAction($Ident, $Value): void
    {
        ...
        if (isset($command)) {
            $deviceCommandUrl =
                self::SERVER_KNM . '/?token=' . $this->ReadAttributeString(self::ATTR_TOKEN_KNM) . $strSerialnumber . $this->GetValue('deviceSN')
    ...
    {
        /* Device S/N */
        $this->updateIfNecessary($device['serial number'], self::VAR_IDENT_DEVICE_SN);
        ...
        $this->updateIfNecessary(
            $this->dezimal2x8bitTo16bitDez($device['data']['Tableread 1'][31], $device['data']['Tableread 1'][30]),
            self::VAR_IDENT_TOTAL_REGENERATION
        );

        /* currentFlow */
        $this->updateIfNecessary(
            $this->dezimal2x8bitTo16bitDez($device['data']['Tableread 2'][11], $device['data']['Tableread 2'][10]),
            self::VAR_IDENT_CURRENTFLOW
        );
        ...
        //Max Entnahmezeit
        $this->updateIfNecessary(
            $this->dezimal2x8bitTo16bitDez($device['data']['Tableread 2'][17], $device['data']['Tableread 2'][16]),
            self::VAR_IDENT_WATERSTOP_MAXPERIODOFUSE
        );

        //Max Menge
        $this->updateIfNecessary(
            $this->dezimal2x8bitTo16bitDez($device['data']['Tableread 2'][15], $device['data']['Tableread 2'][14]),
            self::VAR_IDENT_WATERSTOP_MAXQUANTITY
        );

        //Max Durchfluss
        $this->updateIfNecessary(
            $this->dezimal2x8bitTo16bitDez($device['data']['Tableread 2'][13], $device['data']['Tableread 2'][12]),
            self::VAR_IDENT_WATERSTOP_MAXWATERFLOW
        );
            ...
            if (isset($json['status']) && ($json['status'] === 'ok')) {
                ...
                switch ($this->ReadPropertyString(self::PROP_DEVICETYPE)) {
                        ...
                        if (in_array($dt, [self::DT_I_SOFT_SAFE_PLUS, self::DT_I_SOFT_K_SAFE_PLUS], true)) {
                            ...
                            $this->updateIfNecessary(
                                match ($dt) {
                                    ...
                                    self::DT_I_SOFT_K_SAFE_PLUS => 'i-soft K SAFE+',
                                    default => 'unknown',
                                },
                                self::VAR_IDENT_DEVICE_TYPE
                            );
                        } else {
                    ...
                    case self::DT_I_SOFT_PLUS:
                        ...
                        $this->SendCommand(self::SERVER_JUDO, [
                            'group'         => 'register',
                        ...
                        $responseMyJudoCom_connect = $this->SendCommand(self::SERVER_JUDO, [
                            'group'         => 'register',
                            'parameter'     => self::DT_I_SOFT_PLUS,
                            'serial number' => $serialnumber,
                            'command'       => 'connect'
                        ...
                        if ($device['wtuType'] === self::DT_I_SOFT_PLUS) {
                            $this->SetStatus(IS_ACTIVE);
                            $this->updateIfNecessary('i-soft plus', self::VAR_IDENT_DEVICE_TYPE);
                        } else {
                            $this->SetStatus(self::STATUS_INST_WRONG_DEVICETYPE);
    ...
    private function Login(): bool
    {
        ...
        $responseMyJudoCom = null;
        if ($deviceType === self::DT_I_SOFT_PLUS) {
        ...
        // Fehlerschwelle: KNM immer nötig; JUDO nur bei i-soft plus
        if (!$responseMyJudoEU || ($deviceType === self::DT_I_SOFT_PLUS && !$responseMyJudoCom)) {
            $this->SetStatus(self::STATUS_INST_AUTHENTICATION_FAILED);
            return false;
        }

        $jsonMyJudoEU  = json_decode($responseMyJudoEU, false);
        $jsonMyJudoCom = ($deviceType === self::DT_I_SOFT_PLUS) ? json_decode($responseMyJudoCom, false) : null;

        // Erfolgsprüfung: KNM muss ok sein; JUDO nur bei i-soft plus
        $knmOk  = isset($jsonMyJudoEU->status) && ($jsonMyJudoEU->status === 'ok');
        $judoOk = ($deviceType !== self::DT_I_SOFT_PLUS) || ($jsonMyJudoCom && isset($jsonMyJudoCom->status) && ($jsonMyJudoCom->status === 'ok'));

        if ($knmOk && $judoOk) {
            if ($knmOk) {
                $this->WriteAttributeString(self::ATTR_TOKEN_KNM, $jsonMyJudoEU->token);
            }
            if ($deviceType === self::DT_I_SOFT_PLUS && $judoOk) {
        ...
        // Nur für i-soft plus: Seriennummer prüfen, Geräte synchronisieren und verbinden
        if ($deviceType === self::DT_I_SOFT_PLUS) {
