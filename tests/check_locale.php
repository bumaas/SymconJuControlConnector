...
$moduleDirs = ['JuControlDevice'];

/*
 * Eigennamen: Gerätebezeichnungen des Herstellers werden nicht übersetzt und stehen
 * deshalb bewusst nicht in der locale.json. Ohne diese Liste meldet der Prüfer sie als
 * fehlende Übersetzung (09.09.2026).
 */
$keineUebersetzung = ['i-soft SAFE+', 'i-soft K SAFE+', 'i-soft plus'];
...
foreach ($moduleDirs as $moduleDir) {
    ...
    foreach ($formTexts as $text => $paths) {
        if (!isset($deTranslations[$text]) && !in_array($text, $keineUebersetzung, true)) {
